# Contributing to aumos-mlops-lifecycle
...
# Start local infrastructure (PostgreSQL, Redis, Kafka, MLflow)
...
```
feat: add canary auto-progression with error-rate gating
fix: resolve race condition in retraining job status update
refactor: extract MLflow namespace logic into dedicated helper
docs: update deployment strategy API reference
test: add integration tests for FeatureStoreService.materialize
...
This prohibition covers both direct and transitive dependencies. Before adding any
new package, verify its entire dependency tree is GPL/AGPL-free.
...
### Key Dependencies in This Repo

| Package | License | Verified |
|---------|---------|---------|
| mlflow | Apache 2.0 | Yes |
| feast | Apache 2.0 | Yes |
| fastapi | MIT | Yes |
| pydantic | MIT | Yes |
| sqlalchemy | MIT | Yes |
...
- Coverage must remain >= 80% for `core/` modules (services, models, interfaces)
